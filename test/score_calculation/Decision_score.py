...
def gpt4_chatbot_score(path, save_dir, ques_type = "Semantic_InfoDes"):
    ...
    for k, v in tqdm(path.items()):
        ...
        for que_id, chat_answer in ques_list.items():
            ...
            TEXT_SCORE = f"""Here are two answers to the same question "{Question}" according to the image, and the Answers1 is "{Standard_Answer}", the Answers2 is "{Generated_Answer}". Please refer to Answer 1 and evaluate Answer 2 for accuracy and conciseness, giving a score between 0 and 10, with accuracy being the most important factor. If the answer is incorrect, the score should be 0; if the answer is accurate and concise, the score can be high, such as 9 or 10; if the answer is accurate but verbose, the score should be lower, such as 5 or 6. For example, if the standard answer is Yes and the generated answer is No, or the standard answer is No and the generated answer is Yes, then the score is 0. Only answer score number without any explanation."""
            output, history = gpt.chat(TEXT_SCORE, image_files=v[0])
            
            score_sum += float(output)
            question_sum += 1
            
            mid_answer_list[que_id] = float(output)
            mid_score_sum += float(output)
            mid_que_num += 1
...
if __name__ == "__main__":
    ...
    test_ques_type_list = ["Attitude", "Tool", "Plan"]
    for test_ques_type in test_ques_type_list:
