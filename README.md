...
* **[2025/12/10]  🚀 Update: Action-Capability Test Samples & Real-World Dynamic UAV Scenarios**
  ...
  * 🎬 **Dynamic Scenario Tasks (Real UAV Frames)**
    ...
    * **Dynamic Rider Tracking Task**: The agent must maintain persistent tracking of a continuously moving rider under illumination changes, occlusions, and distractions, adapting its strategy based on evolving visual cues. Performance is measured by **average perception/decision scores across nodes**, reflecting stability, adaptability, and decision consistency.
    ...
    The newly added test dataset is also available at the following link ([https://huggingface.co/datasets/GuoMN/BEDI](https://huggingface.co/datasets/GuoMN/BEDI)), under the `BEDI-1.5` directory. In addition, the corresponding evaluation code has been uploaded to the `test` folder. Additionally, we have improved the file organization under the `test` directory: All scripts related to task testing are now located in the `task_testing` folder. All files for computing and analyzing test result accuracy statistics are placed in the `score_calculation` folder.
